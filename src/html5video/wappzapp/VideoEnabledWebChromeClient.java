 ...
 * - The invoking activity must call html5video.wappzapp.VideoEnabledWebChromeClient's onBackPressed() inside of its own onBackPressed().
...
public class VideoEnabledWebChromeClient extends WebChromeClient implements OnPreparedListener, OnCompletionListener, OnErrorListener
{
     ...
     * @param activityVideoView A ViewGroup in the activity's layout that will display the video. Typically you would like this to fill the whole layout.
     * @param loadingView A View to be shown while the video is loading (typically only used in API level <11). Must be already inflated and without a parent view.
     * @param webView The owner html5video.wappzapp.VideoEnabledWebView. Passing it will enable the html5video.wappzapp.VideoEnabledWebChromeClient to detect the HTML5 video ended event and exit full-screen.
  ...
  public VideoEnabledWebChromeClient(VideoEnabledWebView webView)
    {
        this.webView = webView;
        this.isVideoFullscreen = false;
    }
     ...
     * @param callback A html5video.wappzapp.VideoEnabledWebChromeClient.ToggledFullscreenCallback callback
     */
    ...
    public void onShowCustomView(View view, CustomViewCallback callback)
    {
        if (view instanceof FrameLayout)
        {
            ...
            else
            {
                ...
                {
                    // Run javascript code that detects the video end and other events that should cause low profile UI, and notifies the interface
                    ...
                    js += "if (_ytrp_html5_video != undefined && _ytrp_html5_video != _ytrp_html5_video_last) {";
                    {
                        ...
                        {
                            js += "_VideoEnabledWebView.notifyVideoLowProfile();"; // Must match Javascript interface name and method of VideoEnableWebView
                        }
                        js += "}";

                        js += "_ytrp_html5_video.addEventListener('ended', _ytrp_html5_video_ended);";
                        js += "_ytrp_html5_video.addEventListener('play', _ytrp_html5_video_low_profile);";
                        js += "_ytrp_html5_video.addEventListener('durationchange', _ytrp_html5_video_low_profile);"; // For the first play
                    }
                    js += "}";
                    webView.loadUrl(js);
                }
    ...
    {
        // This method must be manually called on video end in all cases because it's not always called automatically.
